...
-- Uses gen_random_uuid() which is available on Supabase (pgcrypto)
...
-- Sessions policies
create policy if not exists "Allow anon insert sessions"
  on public.sessions
  for insert
  to anon, authenticated
  with check (true);

create policy if not exists "Allow anon update sessions"
  on public.sessions
  for update
  to anon, authenticated
  using (true)
...
-- Results policies
create policy if not exists "Allow anon insert results"
  on public.results
  for insert
  to anon, authenticated
  with check (true);
